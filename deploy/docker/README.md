...
- [generate-auth-env.sh](all-in-one/generate-auth-env.sh)
...
`generate-auth-env.sh` is the recommended way to initialize or patch the local
`.env` file with generated secrets and bundled defaults such as:

- `BOOTSTRAP_FIRST_USER_SECRET`
- `BOOTSTRAP_PROXY_ID`
- `BOOTSTRAP_PROXY_KEY`
...
- `ALL_IN_ONE_PROXY_HTTPS_PORT`
- `ALL_IN_ONE_PUBLIC_ORIGIN`
- `POSTGRES_PORT`
- `POSTGRES_DB`
- `POSTGRES_PASSWORD`

Recommended additional values for the current bundled stack:

- `BOOTSTRAP_FIRST_USER_SECRET`
  - generated by `generate-auth-env.sh`; required by the setup UI before creating the first user
- `OPENAI_API_KEY`
  - required when `INTELLIGENCE_STUB_MODE=false` and you want the bundled intelligence service to run in live mode
...
```json
{
  "resource": "https://<host>:8443/api/mcp",
...
- `/api/mcp` is being routed to the dashboard instead of the API

Check Caddy logs for:

- `POST /api/mcp` going to `app:3000` is correct
- `POST /api/mcp` going to `app:3001` is wrong
